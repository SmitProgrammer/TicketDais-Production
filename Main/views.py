    ...
    try:
        if request.method == "POST":
            ...
            if name == "" or email == "" or phone == "" or psw == "" or c_psw == "":
                ...
                messages.warning(
                    request=request, message="Password not matched ")
            elif len(phone) > 10:
        ...
        try:
            ...
            if created:
                if request.method == "POST" and request.POST['form'] == "1":
                    ...
                    messages.success(
                        request, f"OTP Sent on {request.POST['email']}")
        ...
        try:
            ...
            try:
                ...
                if request.POST['check'] == "done":
                    messages.success(
                        request=request, message="Account Created")
                    os.remove("tmp/" + request.POST['email'])
                    return redirect("/")
        except Exception as e:
            print(e)
...
def forgot_psw(request):
    """Backend for the forgot password page"""
    if request.method == "POST":
        if not request.POST['email'] == "":
            try:
                auth.send_password_reset_email(request.POST['email'])
                messages.success(
                    request=request, message="Password Reset Link Has Been Sent To Your Email Address")
            except Exception as e:
...
def edit(request):
    """Backend for reset password page"""
